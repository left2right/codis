...
func main() {
	const usage = `
Usage:
	codis-topom [--ncpu=N] [--config=CONF] [--log=FILE] [--log-level=LEVEL] [--host-admin=ADDR] [--pidfile=FILE] [--join=ADDR]
...
`
	...
	if s, ok := utils.Argument(d, "--join"); ok {
		config.Join = s
		if err := topom.PrepareJoinCluster(config); err != nil {
			log.PanicErrorf(err, "Join topom etcd cluster failed\n%s", err)
		}
	}

	s, err := topom.New(config)
	if err != nil {
	...
	go func() {
		...
		sig := <-c
		log.Warnf("[%p] codis-topom receive signal = '%v'", s, sig)
	}()

	for i := 0; !s.IsClosed() && !s.IsOnline(); i++ {
		if err := s.Start(true); err != nil {
			if i <= 15 {
				log.Warnf("[%p] codis-topom online failed [%d]", s, i)
			} else {
				log.Panicf("codis-topom online failed, give up & abort :'(")
			}
			time.Sleep(time.Second * 2)
		}
	}

	log.Warnf("[%p] codis-topom is working ...", s)

	for !s.IsClosed() {
		time.Sleep(time.Second)
	}

	log.Warnf("[%p] codis-topom is exiting ...", s)
}
