...
func PrepareJoinCluster(cfg *Config) error {
	...
	// - A failed topom re-joins the previous cluster.
	if existed {
		return errors.New("missing data or join a duplicated topom")
	}

	// - A new topom joins an existing cluster.
	// - A deleted topom joins to previous cluster.
	...
	for _, memb := range listResp.Members {
		...
		for _, m := range memb.PeerURLs {
			topoms = append(topoms, fmt.Sprintf("%s=%s", n, m))
		}
	}
	initialCluster = strings.Join(topoms, ",")
	cfg.InitialCluster = initialCluster
	cfg.InitialClusterState = embed.ClusterStateFlagExisting
	return nil
}
